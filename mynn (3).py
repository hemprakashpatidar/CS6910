...
for i in range(100):
  if(isPresent[train_labels[i]]==0):
    ...
    oplabel.append(label_array[train_labels[i]])
  if(count==0):
    break

"""Question 1"""

wandb.log({"Example": [wandb.Image(img,caption=caption) for img,caption in zip(opimg,oplabel)]})
...
sweep_config = {
    ...
    'parameters': {
        ...
        'optimiser': {
            'values': ['sgd','sgdmom','rmsprop','adam']
        },
        'initialisation': {
...
def train_m():
  run = wandb.init()
  config=run.config
  model = MyDNN(3,[128,128,128,10],run.config.learning_rate,0.9,0.999,run.config.epochs,run.config.Batch_size,run.config.initialisation,run.config.activationfn,'softmax', config.optimiser)
